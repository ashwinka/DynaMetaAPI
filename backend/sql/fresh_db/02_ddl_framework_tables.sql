...
CREATE TABLE IF NOT EXISTS APP_RULES(
	ID              VARCHAR(36)     NOT NULL PRIMARY KEY,
    MODULE_ID       VARCHAR(300)    NOT NULL REFERENCES MODULE(MODULE_ID),
    RULE_ID         VARCHAR(300)    NOT NULL UNIQUE,
	VERSION			INT				NOT NULL DEFAULT 0,
	DESCRIPTION		VARCHAR(1000),	
	STATUS			VARCHAR(50)		NOT NULL DEFAULT 'DRAFT' CHECK (STATUS IN ('DRAFT','PUBLISHED')),
	IS_ACTIVE		VARCHAR(500)	NOT NULL DEFAULT 'N' CHECK (IS_ACTIVE IN ('Y','N')),
	IS_OOB			VARCHAR(1)		NOT NULL DEFAULT 'N' CHECK (IS_OOB IN ('Y','N')),
	RULE_TYPE		VARCHAR(50)		NOT NULL CHECK (RULE_TYPE IN ('CONDITIONAL','VALIDATION', 'RENDERER', 'DERIVATIONS', 'WORKFLOW')),
	
	TRIGGER_FIELDS	VARCHAR(1000),	
